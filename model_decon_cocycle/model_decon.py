...
class Model_Decon(object):
    ...
    def __init__(self, sess, opts):
        ...
        # Initialize weights for cocycle adjustment
        self.w_cocycle = tf.get_variable('w_cocycle', [self.opts['z_dim'], self.opts['z_dim']], tf.float32,
                                         initializer=tf.contrib.layers.xavier_initializer())
        self.b_cocycle = tf.get_variable('b_cocycle', [self.opts['z_dim']], tf.float32,
                                         initializer=tf.constant_initializer(self.opts['init_bias']))
        self.w_a = tf.get_variable('w_a', [self.opts['a_dim'], self.opts['z_dim']], tf.float32,
                                   initializer=tf.contrib.layers.xavier_initializer())
...
########################################################################################################################
    ...
    def cocycle_adjustment(self, z_prev, a_prev):
        # Reshape z_prev and a_prev to 2D for matrix multiplication
        z_prev_reshaped = tf.reshape(z_prev, [-1, z_prev.get_shape().as_list()[-1]])  # [batch_size * nsteps, z_dim]
        a_prev_reshaped = tf.reshape(a_prev, [-1, a_prev.get_shape().as_list()[-1]])  # [batch_size * nsteps, a_dim]

        # Perform the matrix multiplication
        z_cocycle = tf.tanh(tf.matmul(z_prev_reshaped, self.w_cocycle) + tf.matmul(a_prev_reshaped, self.w_a) + self.b_cocycle)

        # Optionally reshape back to 3D if needed
        z_cocycle = tf.reshape(z_cocycle, [self.opts['batch_size'], -1, self.w_cocycle.get_shape().as_list()[-1]])

        return z_cocycle

    def p_z_g_z_a(self, z, a):
        # Cocycle adjustment
        z_cocycle = self.cocycle_adjustment(z, a)
        z_fea = fc_net(self.opts, z_cocycle, self.opts['pzgz_net_layers'], self.opts['pzgz_net_outlayers'], 'pzgz_net')
...
########################################################################################################################
    def train_model(self, data):
        ...
        log_file = os.path.join(log_dir, 'cocycle.log')
