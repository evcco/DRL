...
def create_mnist_datasets():
    ...
    mask_dim = 1
    u_dim = 1  # Changed from 2 to 1 to match the model's expectation
    ...
    def collect_data(images, num_samples):
        ...
        for i in range(num_samples):
            ...
            for _ in range(nsteps):
                ...
                ep_masks.append([1.0])
                ep_rich.append([np.random.randint(0, 2)])  # Changed to generate one random Bernoulli value for rich
    ...
    np.savez('mnist_training_data_1.npz', x_train=x_train, a_train=a_train, r_train=r_train, mask_train=mask_train, rich_train=rich_train)
    np.savez('mnist_validation_data_1.npz', x_validation=x_validation, a_validation=a_validation, r_validation=r_validation, mask_validation=mask_validation, rich_validation=rich_validation)
    np.savez('mnist_testing_data_1.npz', x_test=x_test, a_test=a_test, r_test=r_test, mask_test=mask_test, rich_test=rich_test)
