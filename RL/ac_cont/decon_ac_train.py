...
class AC_Decon(object):
    ...
    def __init__(self, sess, opts, model):
        ...
        ########################################## Integrating Surrogate Model and Front-Door Adjustment ################

        # Surrogate model to learn residuals
        with tf.variable_scope('surrogate_net'):
            self.surrogate_residual_mu, self.surrogate_residual_sigma = \
                self.surrogate_net(self.z_ph, self.a_ph, False, self.is_training_ph, True)

        # Front-door adjustment integration
        self.adjusted_action_mu, self.adjusted_action_sigma = \
            self.frontdoor_adjustment(self.z_ph, self.surrogate_residual_mu, self.surrogate_residual_sigma)
    ...
    def surrogate_net(self, z, a, reuse, is_training, trainable):
        # Surrogate network for learning residuals
        z_a = tf.concat([z, a], axis=1)
        mu, sigma = ac_fc_net(self.opts, z_a, self.opts['surrogate_net_layers'],
                              self.opts['surrogate_net_outlayers'], 'surrogate_net',
                              reuse=reuse, is_training=is_training, trainable=trainable)
        return mu, sigma

    def frontdoor_adjustment(self, z, residual_mu, residual_sigma):
        # Adjust action based on learned residuals from the surrogate model
        adjusted_mu = self.a_mu + residual_mu
        adjusted_sigma = self.a_sigma + residual_sigma
